...
function [max_intensity_l, max_intensity_r, lineWidth, ann_rect, lineColour] = check_luminance_r()
...
while KeyCode(EscapeKey)==0
    image_mono_h(:,:,3) =  (bgint_r + (sin(y+phase_r)+1)*(max_intensity_r-bgint_r)/2) .* circle;
    ...
    [keyIsDown, press, KeyCode] = KbCheck;
    if KeyCode(UpKey)==1 && max_intensity_r < 255
        max_intensity_r = max_intensity_r + 1
    elseif KeyCode(DownKey)==1 && max_intensity_r >0
        max_intensity_r = max_intensity_r - 1
    end
end

filename = ['stimulus/' subj '/luminance_r_' subj '.mat'];
save(filename, 'max_intensity_r')
